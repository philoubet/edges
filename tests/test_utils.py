...
def test_safe_eval():
    ...
    assert safe_eval("[x * 2 for x in range(3)]", SAFE_GLOBALS=SAFE_GLOBALS, parameters=params) == [0, 2, 4]

def test_safe_eval_cached():
    SAFE_GLOBALS = {"sum": sum}
    params = {}
    assert safe_eval_cached("10 + 5", SAFE_GLOBALS=SAFE_GLOBALS, parameters=params, scenario_idx="foo") == 15
    assert safe_eval_cached("sum([1, 2, 3])", SAFE_GLOBALS=SAFE_GLOBALS, scenario_idx="foo", parameters=params) == 6
